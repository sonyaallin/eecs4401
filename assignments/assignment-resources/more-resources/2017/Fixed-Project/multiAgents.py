...
import random, util
from math import log, sqrt # imported the functiions for the math ops
import datetime
...
class MonteCarloAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        percent_wins, move = max(
            (self.wins.get((gameState, player), 0) / self.plays.get((gameState, player), 1), action)
            for action, gameState in movesStates
        )
        
        for x in sorted(
           ((100 * self.wins.get((gameState, player), 0) /
             self.plays.get((gameState, player), 1),
             self.wins.get((gameState, player), 0),
             self.plays.get((gameState, player), 0), action)
            for action, gameState in movesStates),
               reverse=True            
        ):
           print "{3}: {0:.2f}% ({1} / {2})".format(*x)
        
        if gameState.isWin() and player == 0:
            print(gameState + " win. Score: " + gameState.getScore())
    ...
    def run_simulation(self, state):
        ...
        * Remember to limit the depth of the search only in the expansion phase!
        ...
        winflag = False #we need these to know if we are back propagating a win or a loss
        ...
        for t in xrange(1, maxMoves + 1):
            ...
            if all(self.plays.get((S, player)) for p, S in legal_moves):
                # If we have stats on all of the legal moves here, use them.

                log_total = log(sum(self.plays[(S, player)] for p, S in legal_moves))
            
                value, move, state = max(
                    ((self.wins[(S, player)] / self.plays[(S, player)]) +
                    self.C * sqrt(log_total / self.plays[(S, player)]), p, S)
            ...
            if state.isWin() or state.isLose(): #THIS WAS THE MISSING BIT.  Each path leads to a a single loss or win.
                if (state.isWin()): 
                    winflag = True
                else:
                    loseflag = True 
        ...
        for state, player in visitedStates:
            ...
            if (winflag): #before state.isWin and state.isLose was being called down here.
                if (player == 0): #update only if player is 0
                    self.wins[(state, player)] += (scoreEvaluationFunction(state) + 500)/1000
            elif (loseflag):
                if (player > 0): #update only if player is not 0
                    self.wins[(state, player)] += 1-((scoreEvaluationFunction(state) + 500)/1000)
            #you can also try updating if not win or lose, with heuristic value
    ...
    def final(self, state):
        ...
        #for s in self.states:
            #if (s, player) in self.plays:
                #self.plays[(s, player)] += 1
                #self.wins[(s, player)] += self.evaluationFunction(state)
        pass
...
def betterEvaluationFunction(currentGameState):
    ...
    for ghost in ghosts:
        ghostDist = manhattanDistance(pacman, ghost)
        closestGhost = min(ghostDist, closestGhost)
        
    for food in foods.asList():
        foodDist = manhattanDistance(pacman, food)
        closestFood = min(foodDist, closestFood)
    
    for capsule in capsules:
        capsuleDist = manhattanDistance(pacman, capsule)
        closestCapsule = min(capsuleDist, closestCapsule)
    
    return score + 10 / (closestGhost + 1) + 10 / (closestFood + 1) + 10 / (closestCapsule + 1)
