...
class TestGetAction(unittest.TestCase):
    ...
    def testTimeOut(self):
        ...
        self.assertEqual(pacman.calculation_time, datetime.timedelta(milliseconds=40))
    ...
    def testSelection(self):
        ...
        max_val = max(
                ((pacman.wins[(state.generateSuccessor(0, a),0)] / pacman.plays[(state.generateSuccessor(0, a),0)]))
                for a in actions)
        max_acts = [a for a in actions if (state.generateSuccessor(0, a),0) in pacman.plays
                    and pacman.plays[(state.generateSuccessor(0, a),0)] != 0 and
                    float(pacman.wins[(state.generateSuccessor(0, a)),0]) /
                    pacman.plays[(state.generateSuccessor(0, a),0)] == max_val]
...
class TestRunSimulation(unittest.TestCase):
    ...
    def testDepth(self):
        ...
        game = self.rules.newGame(map, pacman, ghosts, textDisplay.NullGraphics(), True, True)
        state = game.state
        pacman.plays[(state,0)] = 1
        pacman.wins[(state,0)] = 0
        states = [state]
        for i in range(2):
    ...
    def testEarlyWin(self):
        ...
        state = game.state
        actions = state.getLegalActions(0)
        pacman.plays[(state,0)] = len(actions) + 1
        pacman.wins[(state,0)] = len(actions)
    ...
    def testNoInfo(self):
        ...
        pacman.plays[(state,0)] = len(succ)
        pacman.wins[(state,0)] = len(succ)
        ...
        try:
            ...
            pacman.plays[(succ[-1],0)] = 0
            pacman.wins[(succ[-1],0)] = 0
            pacman.run_simulation(state)
        # Should play the single state without information
        self.assertEqual(pacman.plays[(succ[-1],0)], 1)
    ...
    def testUCB1(self):
        ...
        state = game.state
        succ = [state.generateSuccessor(0, a) for a in state.getLegalActions(0)]
        pacman.plays[(state,0)] = len(succ) + 1
        pacman.wins[(state,0)] = len(succ)
        pacman.plays[(succ[0],0)] = 1
        pacman.wins[(succ[0],0)] = 1
        for i in range(1, len(succ)):
            pacman.plays[(succ[i],0)] = 1
            pacman.wins[(succ[i],0)] = 0

        pacman.run_simulation(state)

        self.assertEqual(pacman.plays[(succ[0],0)], 2)
    ...
    def testUCB2(self):
        ...
        state = game.state
        succ = [state.generateSuccessor(0, a) for a in state.getLegalActions(0)]
        pacman.plays[(state,0)] = len(succ) + 2
        pacman.wins[(state,0)] = len(succ) + 2
        pacman.plays[(succ[i],0)] = 1
        pacman.wins[(succ[i],0)] = 0
        for i in range(1, len(succ)):
            pacman.plays[(succ[i],0)] = 2
            pacman.wins[(succ[i],0)] = 0
        pacman.run_simulation(state)
        self.assertEqual(pacman.plays[(succ[0],0)], 2)
