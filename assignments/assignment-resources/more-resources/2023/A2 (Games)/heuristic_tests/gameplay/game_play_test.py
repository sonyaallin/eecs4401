#!/usr/bin/env python
import sys
from mancala_game import MancalaGameManager, AiPlayerInterface, play_game
from agent import compute_heuristic
from agent_TA_default import compute_utility
...
boards = []
with (open("/Users/JaglalLab/Desktop/HW/A3/submissions/solution/boards.pickle", "rb")) as openfile:
    while True:
        try:
...
def test_heuristic(efile, name):
    ...
    for i in range(len(boards)):
        ...
        if val != 0 and abs(board.mancalas[0]-board.mancalas[1]) > 0: okflag = True
        if val == 0 and abs(board.mancalas[0]-board.mancalas[1]) == 0: okflag = True

        signal.alarm(timeset)  # Two minutes per game MAX
        try:
            val = compute_heuristic(board, 0)
        except Exception as e:
            output = str(e)
            output = output.replace(",", "-")
            efile.write("{}, Exception encountered -- {}\n".format(name, output))
    ...
    #5 different depths
    totals = []
    for i in range(1, n): #depths from 4 to 8

        p1 = AiPlayerInterface(agent1, 1, i+3, minimax, caching)
        p2 = AiPlayerInterface(agent2, 0, i+3, minimax, caching)
    ...
    # #rotate player order
    # for i in range(1, n):
    #     p1 = AiPlayerInterface(agent2, 1, i + 2, minimax, ordering)
    #     p2 = AiPlayerInterface(agent1, 0, i + 2, minimax, ordering)
