...
class AiPlayerInterface(Player):
    ...
    def get_move(self, manager):
        ...
        eprint("SCORE|{}|{}|{}|{}\n".format(white_score, dark_score, str(manager.board.pockets), str(manager.board.mancalas)).encode("ASCII"))
...
def play_game(game, player1, player2):
    ...
    while True: 
        ...
        if not possible_moves: 
            ...
            eprint("FINAL: {} (dark) {}:{} {} (light)".format(player1.name, p1score, p2score, player2.name))
            player1.kill(game)
            player2.kill(game)
            return (player1.name, p1score, p2score, player2.name)
        else:
            #try: 
            eprint("game.current_player is: " + str(game.current_player))
            i = player_obj.get_move(game)
            game.play(i, game.current_player)
            # except AiTimeoutError:
            #     print("{} ({}) timed out!".format(player_obj.name, color))
            #     player1.kill(game)
            #     player2.kill(game)
            #     if player_obj.name == player1.name:
            #         return (player1.name, -99, -99, -99)
            #     else:
            #         return (player2.name, -99, -99, -99)
