...
def compute_utility(board, side):
    ...
    if side == 0:
        # print("player is 0")
        # print(board.mancalas[side])
        # print(board.mancalas[1])
        return board.mancalas[side] - board.mancalas[1]
    # print("player is 1")
    # print(board.mancalas[side])
    # print(board.mancalas[0])
    return board.mancalas[side] - board.mancalas[0]
...
def compute_heuristic(board, color):
    ...
    """
    opp = abs(color-1)
    val_cur = board.mancala[color]
    val_opp = board.mancala[opp]
    for pocket in board.pockets[color]:
        if pocket == 0:
            val_cur += 2
    for pocket in board.pockets[opp]:
        if pocket == 0:
            val_opp += 2
...
def select_move_minimax(board, color, limit=-1, caching = False):
    ...
    """
    move , _= minimax(board, color, limit,caching)
    return move
    raise RuntimeError("Method not implemented")


def minimax(board, color, depth, caching):
    opp = abs(color-1)
    if depth == 0:
        return None, board
    ...
    for move in get_possible_moves(board,color):
        new_board = Board(board.pockets, board.mancalas)
        new_board = play_move(new_board,color, move)
        if caching:
            if new_board not in cache:
                cache[new_board] = compute_utility(new_board,color)
            else:
                continue
        opp_move, new_board = minimax(new_board,opp,depth-1, caching)
        if caching:
            if new_board not in cache:
                cache[new_board] = compute_utility(new_board,color)
            else:
...
# def minimax_max(board,depth,color):
#     # let 0 be max, A1B0
#     opp= abs(color-1)
#     if depth == 0:
#         return compute_utility(board, color)
#     if end_game(board, color):
#         return compute_utility(board, color)     
#     max_score = -math.inf
#     best_move = None
#     for move in get_possible_moves(board, color):
#         new_board = Board(board.pockets, board.mancalas)
#         new_board = play_move(new_board, color, move)
#         #next_score = max(minimax_min(new_board, depth-1, opp))
#         next_score = minimax_min(new_board, depth-1, opp)
#         if next_score >= max_score:
#             max_score = next_score
#             #best_move = move
#     return max_score

# def minimax_min(board,depth,color):
#     opp= abs(color-1)
#     if depth == 0:
#         return compute_utility(board, color)
#     if end_game(board, color):
...
#         #next_score = min(minimax_max(new_board, depth-1, opp))
#         next_score = minimax_max(new_board, depth-1, opp)
#         if next_score <= min_score:
#             min_score = next_score##############################you hua
#             #best_move = move
#     return min_score
...
def select_move_alphabeta(board, color, limit=-1, caching = False):
    ...
    alpha = -math.inf
    beta = math.inf
    move , _= ab_help(board, color, limit,alpha,beta,caching)
    return move
    raise RuntimeError("Method not implemented")


def ab_help(board, color, depth,alpha, beta,caching):
    opp = abs(color-1)
    if depth == 0:
        return None, board
    if len(get_possible_moves(board,color))==0:
    ...
    for move in get_possible_moves(board,color):
        ...
        if caching:
            if new_board not in cache:
                ...
                continue
        opp_move, new_board = ab_help(new_board,opp,depth-1,alpha,beta,caching)
...
# def pruning_max(board,depth,color,alpha,beta):
#     # let 0 be max, A1B0
#     if depth == 0:
...
#         next_score = max(pruning_min(new_board, depth-1,next_color,alpha,beta))
#         if next_score > max_score:
#             max_score = next_score##############################you hua
#             best_move = move
...
#         #     return best_move
#     return best_move
#         #return max(minimax_max(new_board))

# def pruning_min(board,depth,color,alpha,beta):
#     if depth == 0:
#         return compute_heuristic(board, color)
#     if end_game(board, color):
#         return compute_utility(board, color)
...
#         new_board = play_move(board, color, move)
#         next_color = abs(color-1)
#         next_score = min(pruning_max(new_board, depth-1,next_color,alpha,beta))
#         if next_score < min_score:
#             min_score = next_score##############################you hua
#             best_move = move
...
def ucb_select(board, mcts_tree):
    ...
    best_score = -math.inf
    best_successor = None
    for successor in mcts_tree.successors.get(board, []):
        visit_count = mcts_tree.counts[successor]
        rewards = mcts_tree.rewards[successor]
        if visit_count == 0:
            visit_count=1
        score = rewards / visit_count + constant * math.sqrt(math.log(mcts_tree.counts[board]) / visit_count)
        if score > best_score:
            best_score = score
