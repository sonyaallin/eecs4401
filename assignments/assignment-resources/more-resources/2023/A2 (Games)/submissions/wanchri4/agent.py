...
def compute_utility(board, side):
    ...
    """
    player1, player2 = board.mancalas
    if side == 0:
...
def compute_heuristic(board, color):
    ...
    # want to keep as many marble as possible onto your side of the board since it means you have a higher chance of
    # converting those marbles into your macala

    # initialization
    mancala1, mancala2 = board.mancalas
    player1 = board.pockets[0]
    player2 = board.pockets[1]
    player1_num_of_marbles = 0
    player2_num_of_marbles = 0

    # calculate the total number of marbles on each side of the board
    for i in range(len(player1)):
        player1_num_of_marbles += player1[i]
        player2_num_of_marbles += player2[i]

    # base on whose turn it is return the different between the utility of the board added with the
    # difference of the number of marbles that exist on the board for each player
    if color == 0:
        return mancala1 - mancala2 + (player1_num_of_marbles - player2_num_of_marbles)
    else:
        return mancala2 - mancala1 + (player2_num_of_marbles - player1_num_of_marbles)
...
def select_move_minimax(board, color, limit=-1, caching = False):
    ...
    """
    def miniMax(board, color, limit, caching, maxTurn):
        ...
        if maxTurn:
            ...
            for move in possible_moves:
                ...
                _, value = miniMax(new_board, color, limit - 1, caching, False)

                if caching:
                    cache[new_board] = (move, value)

                if value > biggest:
                    best_max_move, biggest = move, value

            return best_max_move, biggest

        else:
            opponent = get_opposite_color(color)
            ...
            # get the move
            for move in possible_moves:
                new_board = play_move(board, opponent, move)
                _, value = miniMax(new_board, color, limit - 1, caching, True)

                if caching:
                    cache[new_board] = (move, value)

                if value < smallest:
                    best_min_move, smallest = move, value

            return best_min_move, smallest

    cache.clear()
    best_move, best_eval = miniMax(board, color, limit, caching, True)
    return best_move
...
def select_move_alphabeta(board, color, limit=-1, caching = False):
    ...
    def alpha_beta_miniMax(board, color, alpha, beta, limit, caching, maxTurn):
        ...
        if maxTurn:
            ...
            for move in possible_moves:
                new_board = play_move(board, color, move)
                _, value = alpha_beta_miniMax(new_board, color, alpha, beta, limit - 1, caching, False)
            ...
            for move in possible_moves:
                new_board = play_move(board, opponent, move)
                _, value = alpha_beta_miniMax(new_board, color, alpha, beta, limit - 1, caching, True)
    ...
    cache.clear()
    best_move, best_eval = alpha_beta_miniMax(board, color, -math.inf, math.inf, limit, caching, True)
    return best_move
...
def ucb_select(board, mcts_tree):
    ...
    '''
    UCB_Sn = rewardSn / total_Sn + bias * sqrt(ln(total_Sn_parent)/ total_Sn)
    '''
    highest = -math.inf
    ans = None
    v_1 = mcts_tree.rewards[board] / mcts_tree.counts[board]

    for successor in mcts_tree.successors[board]:
        calc = mcts_tree.weight * (math.sqrt(math.log(len(mcts_tree.successors[board]))/mcts_tree.counts[board]))
        total = v_1 + calc
        if total > highest:
            ans = successor
            highest = total
