...
def prop_BT(csp, newVar=None):
    ...
    for c in csp.get_cons_with_var(newVar):
        if c.get_n_unasgn() == 0: # if all variables are assigned for that constraint
            vals = []
            vars = c.get_scope()
            for var in vars:
...
def ord_mrv(csp):
    ...
    for var in variables:
        if (not var.is_assigned()):
            if (min_var is None):
                min_curr_dom = var.cur_domain_size()
                min_var = var
            elif (var.cur_domain_size() < min_curr_dom):
                min_curr_dom = var.cur_domain_size()
                min_var = var
    return min_var
