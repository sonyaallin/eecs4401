    ...
    for c in cons:
        if c.get_n_unasgn() == 1:
            ...
            for i in range(len(scope)):
                v = scope[i].get_assigned_value()
                if v:
                    vals.append(v)
                else:
                    unassigned = scope[i]
                    domain = unassigned.cur_domain()
                    vals.append(domain[0])
                    index = i
            ...
            for i in range(len(domain)):
                ...
                if not c.check(vals):
                    num_pruned += 1
                    pruned.append((unassigned, domain[i]))
                    unassigned.prune_value(domain[i])
            if num_pruned == len(domain):
                return False, pruned
...
def prop_GAC(csp, newVar=None):
    ...
    while constraint:
        variables = constraint.get_unasgn_vars()
