...
def prop_FC(csp, newVar=None):
    ...
    pruned = []
    if not newVar:  # check all constraints
        constraints = csp.get_all_cons()
    else:  # check constraints only containing newVar
        constraints = csp.get_cons_with_var(newVar)
    for c in constraints:
        if c.get_n_unasgn() == 1:  # only one uninst
            vars = c.get_scope()
            for var in vars:
                if not var.is_assigned():  # this var is not unassigned, variable X
                    vals = var.cur_domain()
                    if vals == []:  # DWO
                        return False, pruned
                    for val in vals:
                        if not c.has_support(var, val):  # this one not satisfy
    ...
    while gacqueue:
        ...
        for var in vars:
            ...
            if vals == []:  # DWO
                return False, pruned
            for val in vals:
                if not c.has_support(var, val):  # for the line A not found
                    if (var, val) not in pruned:
                        var.prune_value(val)
                        pruned.append((var, val))
                    if var.cur_domain() == []:  # DWO
                        ...
                        for cbar in constraints:
                            if cbar not in gacqueue:
                                gacqueue.append(cbar)
    return True, pruned
