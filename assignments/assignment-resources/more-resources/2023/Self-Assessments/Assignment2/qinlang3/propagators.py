...
def prop_FC(csp, newVar=None):
    ...
    pruned = []
    if not newVar:
        csp_lst = csp.get_all_cons()
    else:
        csp_lst = csp.get_cons_with_var(newVar)
    for c in csp_lst:
        if c.get_n_unasgn() == 1:
            vals = []
            vars = c.get_scope()
            ...
            for var in vars:
                ...
                vals.append(var.get_assigned_value())
                count += 1
            for val in vars[idx].cur_domain():
                vals[idx] = val
                if not c.check(vals):
                    vars[idx].prune_value(val)
                    pruned.append((vars[idx], val))
            if vars[idx].cur_domain_size == 0:
                return False, pruned
    return True, pruned            

def has_solution(vals, c):
    idx = -1
