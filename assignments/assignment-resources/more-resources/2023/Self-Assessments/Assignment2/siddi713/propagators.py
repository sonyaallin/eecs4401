...
def prop_FC(csp, newVar=None):
  ...
  # IMPLEMENT
  if not newVar:
    return forward_checking(csp.get_all_cons())
  return forward_checking(csp.get_cons_with_var(newVar))

def forward_checking(constraints):
  pruned = []
...
def prop_GAC(csp, newVar=None):
  ...
  if not newVar:
    return GAC(csp, csp.get_all_cons())
  return GAC(csp, csp.get_cons_with_var(newVar))

def GAC(csp, constraints):
  pruned = []
  q = deque(constraints)
  while len(q) > 0:
    con = q.popleft()
