        ...
        for constraint in csp.get_all_cons():
            if constraint.get_n_unasgn() == 1:
                ...
                for value in var.cur_domain():
                    ...
                    if not constraint.check(tuple(lst)):
                        var.prune_value(value)
                        pruned_lst.append((var, value))
...
def prop_GAC(csp, newVar=None):
    ...
    while queue != []:
        curr_con = queue.pop(0)
        for var in curr_con.get_scope():
            for value in var.cur_domain():
                if not curr_con.has_support(var, value):
                    ...
                    if var.cur_domain_size() == 0:
                        ...
                        for new_con in csp.get_cons_with_var(var):
                            if new_con not in queue and new_con != curr_con:
