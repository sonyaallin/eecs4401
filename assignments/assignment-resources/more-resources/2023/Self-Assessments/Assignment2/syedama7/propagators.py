    ...
    for constraint in constraints:
        if constraint.get_n_unasgn() == 1:
            ...
            for x in unassigned_var.cur_domain():
                if not constraint.has_support(unassigned_var, x):
                    if (unassigned_var, x) not in pruned:
                        pruned.append((unassigned_var, x))
...
def prop_GAC(csp, newVar=None):
    ...
    while len(gac_queue) != 0:
        nongac_c = gac_queue.pop()
        for v in nongac_c.get_scope():
            for x in v.cur_domain():
                if not nongac_c.has_support(v, x):
                    if (v, x) not in pruned:
                        pruned.append((v, x))
                        v.prune_value(x)
...
def ord_mrv(csp):
    ...
    # IMPLEMENT
    min_curdom = min([v.cur_domain_size() for v in csp.get_all_unasgn_vars()])
    for v in csp.get_all_unasgn_vars():
        if v.cur_domain_size() == min_curdom:
            return v
    return None
