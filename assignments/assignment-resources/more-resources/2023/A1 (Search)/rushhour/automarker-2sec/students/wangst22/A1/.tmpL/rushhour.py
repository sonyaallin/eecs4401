...
class Rushhour(StateSpace):
    ...
    def get_print_state(self):
        ...
        result = ""
        if self.parent:
            result += "Action= \"{}\", S{}, g-value = {}, (From S{})".format(self.action, self.index, self.gval, self.parent.index)
        else:
            result += "Action= \"{}\", S{}, g-value = {}, (Initial State)".format(self.action, self.index, self.gval)

        result += "Vehicle Statuses"
        for vs in sorted(self.get_vehicle_statuses()):
            result += "    {} is at ({}, {})\n".format(vs[0], vs[1][0], vs[1][1])
        board = get_board(self.get_vehicle_statuses(), self.get_board_properties())
        result += '\n'
        result += '\n'.join([''.join(board[i]) for i in range(len(board))])
        return result
...
# RUSH HOUR GOAL TEST
def rushhour_goal_fn_ta(state): #IMPLEMENT

    if not isinstance(state,Rushhour):
...
# #Rush hour GOAL TEST
# def rushhour_goal_fn_final(state): #IMPLEMENT
...
# def heur_min_moves_final(state):
