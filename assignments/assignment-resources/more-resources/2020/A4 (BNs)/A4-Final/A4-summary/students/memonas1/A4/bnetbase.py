...
def same_len_helper(var_dom_list, Factors):
    """
    helper function
    :param var_dom_list:
    :type var_dom_list:
...
def diff_len_helper(var_dom_list, var_list, var_raw_list, Factors):
    """
    helper function
...
def multiply_factors(Factors):
    ...
    var_dom_list = list(map(lambda vs: list(map(Variable.domain, vs)), var_raw_list))

    if len(var_set) == len(var_list):
        vals = same_len_helper(var_dom_list, Factors)

    else:

        vals, var_list = diff_len_helper(var_dom_list, var_list, var_raw_list, Factors)

    f = Factor(var_name, var_list)
    f.add_values(vals)
    return f
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    Z = min_fill_ordering(FF, QueryVar)
    for z in Z:
    ...
    if total == 0:
        for val in dist:
            to_return.append(float('inf'))
    else:
        for val in dist:
            to_return.append(val/total)
    return to_return
