...
def check_assign(assign, var_list):
    "add all the values to the dict and return the dict"
    dict1 = {}
...
def multiply_factors(Factors):
    ...
    vrl = list(map(Factor.get_scope, Factors))
    vl = sum(vrl, [])
    var_set = set(vl)
    var_name = "Mult[" + " + ".join(f.name for f in Factors) + "]"
    vdl = list(map(lambda vs: list(map(Variable.domain, vs)), vrl))

    # if they have the same length
    if len(var_set) == len(vl):
        #vals = helper1(vdl, Factors)
        poss = (product(*f) for f in vdl)
        ...
        for assign in all_poss:
            ...
            vals_lst.append(assign_list + [val])

        vals = vals_lst

    else:
        #vals, vl = helper2(vdl, vl, vrl, Factors)
        already_assign = set()
        temp_var_list = []
        for v in vl:
            if v not in temp_var_list:
        ...
        vals = []
        for assign in all_poss:
            good_assign = check_assign(assign, vrl)
            if not good_assign:
                continue
        ...
        vals, vl = vals, var_list

    f = Factor(var_name, vl)
    f.add_values(vals)
    return f
...
def sum_out_variable(f, var):
    ...
    vl = f.get_scope()
    idx = vl.index(var)
    var_list_new = [v for v in vl if v != var]
    fac = Factor(f.name + "[S:" + var.name + "]", var_list_new)
    ...
    for assign in product(*domain_new):
        ...
        for v in var.domain():
            ...
            val_count += f.get_value(assign_list)
        vals.append(list(assign) + [val_count])
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    Z = min_fill_ordering(f_list, QueryVar)
    for z in Z:
        fs = []
        for ff in f_list:
            if z in ff.get_scope():
                fs.append(ff)
        g = sum_out_variable(multiply_factors(fs), z)
        f_list = [ff for ff in f_list if ff not in fs]
        f_list.append(g)
