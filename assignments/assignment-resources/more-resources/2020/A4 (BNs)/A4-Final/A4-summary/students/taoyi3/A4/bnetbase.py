...
def restrict_factor(f, var, value):
    ...
    result = Factor(f.name + "[R:" + var.name + "]", var_list_new)
...
def sum_out_variable(f, var):
    ...
    #IMPLEMENT
    var_list_new = [v for v in f.get_scope() if v != var]
    result = Factor(f.name + "[S:" + var.name + "]", var_list_new)
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    for f in Net.factors():
        restrict_vars = set(EvidenceVars).intersection(f.get_scope())
        if len(restrict_vars) > 0: # f can be restricted
            ff = f
            for ef in restrict_vars:
                ff = restrict_factor(ff, ef, ef.get_evidence())
            factor_lst.append(ff)
        else:
            factor_lst.append(f)

    for ordering in min_fill_ordering(factor_lst, QueryVar):
        fs = [ff for ff in factor_lst if ordering in ff.get_scope()]
        g = sum_out_variable(multiply_factors(fs), ordering)
        factor_lst = [ff for ff in factor_lst if ff not in fs]
        factor_lst.append(g)

    dist = [multiply_factors(factor_lst).get_value([v]) for v in QueryVar.domain()]
    total = sum(dist)
    if total == 0:
