    ...
    for f in Factors:
        for v in f.get_scope():
            if v not in new_scope:
                new_scope.append(v)
    sep = '*'
    new_name = sep.join([f.name for f in Factors])
    new_factor = Factor(new_name, new_scope)
    doms = [v.domain() for v in new_scope]
    combs = itertools.product(*doms)
    for c in combs:
        for i in range(len(new_scope)):
            new_scope[i].set_assignment(c[i])
        value = 1
        for f in Factors:
...
def sum_out_variable(f, var):
    ...
    new_scope = f.get_scope()
    new_scope.remove(var)
    new_name = f.name + '_sum_' + var.name
    new_factor = Factor(new_name, new_scope)
    doms = [v.domain() for v in new_scope]
    combs = itertools.product(*doms)
    for c in combs:
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    for f in Net.factors():
        ...
        rs_factors.append(f)
    ordering = min_fill_ordering(rs_factors, QueryVar)
    for v in ordering:
        factors_lst = []
        for f in rs_factors:
            if v in f.get_scope():
                factors_lst.append(f)
        for f in factors_lst:
            rs_factors.remove(f)
        new_factor = sum_out_variable(multiply_factors(factors_lst), v)
        rs_factors.append(new_factor)
    final_f = multiply_factors(rs_factors)
    norm_const = sum_out_variable(final_f, QueryVar).get_value([])
    return_lst = []
    for val in QueryVar.domain():
        QueryVar.set_assignment(val)
        normalized_pr = final_f.get_value_at_current_assignments() / norm_const
        return_lst.append(normalized_pr)
    return return_lst
