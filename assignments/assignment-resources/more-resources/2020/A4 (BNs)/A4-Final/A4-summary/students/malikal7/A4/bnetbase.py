...
class Factor: 
    ...
    def print_table(self):
        ...
        savedVals = []  #save and then restore the variable assigned values.
        for v in self.scope:
...
def multiply_factors(Factors):
    ...
    if len(Factors) == 1:
        ...
        newName = Factors[0].name + "_multiply_" + Factors[1].name
        mergedVars = Factors[0].get_scope()
        for var in Factors[1].get_scope():
            if var not in Factors[0].get_scope():
        ...
        multiplyFactorsRecursion(Factors[0], Factors[1], mergedVars, newValues)
...
def multiplyFactorsRecursion(f1, f2, mergedVars, newValues):
    if len(mergedVars) == 0:
        ...
        for val in mergedVars[0].domain():
            ...
            multiplyFactorsRecursion(f1, f2, mergedVars[1:], newValues)
...
def restrict_factor(f, var, value):
    ...
    restrictFactorRecursion(f, newScope, newValues)
...
def sum_out_variable(f, var):
    ...
    for val in var.domain():
        ...
        sumOutVarRecursion(f, newScope, newScopeCopy, newValues)
...
def sumOutVarRecursion(oldFactor, newScope, newScopeCopy, newValues):
    if len(newScope) == 0:
        index = 0
        ...
        for val in newScope[0].domain():
            newScope[0].set_assignment(val)
            sumOutVarRecursion(oldFactor, newScope[1:], newScopeCopy, newValues)
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    mfOrdering = min_fill_ordering(factors, QueryVar)
    for var in mfOrdering:
        factorsToMultiply = []
        for f in factors:
        ...
        mfOrdering = mfOrdering[1:]

    multipliedFactor = multiply_factors(factors)

    # normalize final distribution
    normalizedDist = normalize(multipliedFactor.values)
