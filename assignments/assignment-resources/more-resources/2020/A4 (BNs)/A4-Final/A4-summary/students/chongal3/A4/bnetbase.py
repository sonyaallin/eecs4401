...
def check_assignments(assign, var_list):
    list1 = {}
...
def multiply_factors(Factors):
    ...
    raw_vars = list(map(Factor.get_scope, Factors))
    var_list = sum(raw_vars, [])
    set_vars = set(var_list)

    var_domain = list(map(lambda vs: list(map(Variable.domain, vs)),
                            raw_vars))

    if len(set_vars) == len(var_list):
        vals = []
        poss = (product(*f) for f in var_domain)
        ...
        vals = []
        for assign in all_poss:
            good_assign = check_assignments(assign, raw_vars)
...
def restrict_factor(f, var, value):
    ...
    index1 = var_list.index(var)
    new_vars = [v for v in var_list if v != var]
    new_factor = Factor(f.name + "[R:" + var.name + "]", new_vars)
...
def sum_out_variable(f, var):
    ...
    domain_new = list(map(Variable.domain, new_vars))
    vals = []
    for assign in product(*domain_new):
        val = 0
        for v in var.domain():
            assign_list = list(assign)
            assign_list.insert(index1, v)
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    evi_set = set(E)
    for f in F:
        restrict_vars = evi_set.intersection(f.get_scope())
        if len(restrict_vars) > 0:
            new_factor = f
            for ef in restrict_vars:
                new_factor = restrict_factor(new_factor, ef, ef.get_evidence())
            new_factors.append(new_factor)
        else:
            new_factors.append(f)

    ordered_factors = min_fill_ordering(new_factors, Q)
    for order in ordered_factors:
        fs = [new_factor for new_factor in new_factors if order in new_factor.get_scope()]
        temp = sum_out_variable(multiply_factors(fs), order)
        new_factors = [new_factor for new_factor in new_factors if new_factor not in fs]
