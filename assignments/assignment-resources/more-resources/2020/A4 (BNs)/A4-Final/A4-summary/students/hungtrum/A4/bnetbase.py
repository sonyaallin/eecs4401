...
def multiply_factors(Factors):
    ...
    def check_assign(assignment, var_list):
        lut = {}
        for i in range(len(assignment)):
            curr_assign = assignment[i]
    ...
    name = "Multi[" + " + ".join(f.name for f in Factors) + "]"
    ...
    if len(var_set) == len(var_list):
        ...
        possib = (product(*f) for f in var_dom_list)
        all_possib = product(*possib)
        vals = []
        for assign in all_possib:
            val = 1
        ...
        vals = []
        for assign in all_possib:
            checked = check_assign(assign, var_raw_list)
            if not checked:
                continue
            ...
            for i in range(len(Factors)):
                ...
                factor_assign = list(map(lambda v: checked[v], dom))
                val *= Factors[i].get_value(factor_assign)
            assign_list = list(map(lambda v: checked[v], var_list))
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    for f in F:
        restrict_vars = E_set.intersection(f.get_scope())
    ...
    if total == 0:
        # division by zero
        return [float('inf') for _val in dist]
    else:
