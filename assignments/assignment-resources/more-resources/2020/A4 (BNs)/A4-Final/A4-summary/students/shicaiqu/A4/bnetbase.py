...
def multiply_factors(Factors):
    ...
    for factor in Factors:
        ...
        for v in factor.get_scope()[:]:
            if v not in var_list:
                var_list.append(v)
        vars_matrix.append(factor.get_scope()[:])
    poss =[]
    for i in vars_matrix:
        poss.append(product(*[j.domain() for j in i]))
    ...
    for assign in all_poss:
        ...
        for i in range(len(assign)):
            ...
            curr_vars = vars_matrix[i]
            for j in range(len(curr_vars)):
                var = curr_vars[j]
                val = curr_assign[j]
                if var not in assigns_no_dup: 
                    assigns_no_dup[var] = val
                elif assigns_no_dup[var] != val: 
                    continue
        val = 1
        for i in range(len(Factors)):
            dom = Factors[i].get_scope()[:]
            factor_assign = [assigns_no_dup[v] for v in dom]
            val *= Factors[i].get_value(factor_assign)
        assign_list = [assigns_no_dup[v] for v in  var_list]
...
def restrict_factor(f, var, value):
    ...
    domain = [i.domain() for i in var_list]
    domain[idx] = [value]
    all_poss = product(*domain)
    vals = []
    for assign in all_poss:
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    mfo = min_fill_ordering(f_list, QueryVar)
    for z in mfo:
        fs = []
        not_list = []
        for ff in f_list:
