...
def multiply_two_factors(f1, f2):
    ...
    multiply_two_factors_helper(f1, f2, mergedVars, newValues)
...
def multiply_two_factors_helper(f1, f2, mergedVars, newValues):
    if len(mergedVars) == 0:
        ...
        for val in mergedVars[0].domain():
            ...
            multiply_two_factors_helper(f1, f2, mergedVars[1:], newValues)
...
def sum_out_variable(f, var):
    ...
    for val in var.domain():
        ...
        sum_out_variable_helper(f, newScope, newScopeCopy, newValues)
...
def sum_out_variable_helper(oldFactor, newScope, newScopeCopy, newValues):
    if len(newScope) == 0:
        index = 0
        ...
        for val in newScope[0].domain():
            newScope[0].set_assignment(val)
            sum_out_variable_helper(oldFactor, newScope[1:], newScopeCopy, newValues)
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
    ...
    newOrder = min_fill_ordering(factors, QueryVar)
    for var in newOrder:
        factorsToMultiply = []
        for f in factors:
    ...
    if sum(multipliedFactor.values) == 0:
        normalized = [float('inf')] * len(multipliedFactor.values)
    else:
        normalized = [i / sum(multipliedFactor.values) for i in multipliedFactor.values]
