...
def all_variable_values(scope, acc):
    if not scope:
        return acc
    ...
    if not acc:
        for d in domain:
            acc.append([d])
        return all_variable_values(scope[1:], acc)
    ...
    for lst in acc:
        ...
        for d in domain:
            new_lst = list(lst)
            new_lst.append(d)
            new_acc.append(new_lst)
    return all_variable_values(scope[1:], new_acc)
...
def multiply_factors(Factors):
    ...
    variable_values_lst = all_variable_values(scope, [])
    if not variable_values_lst:
        value = 1
        for factor in Factors:
            if not factor.get_scope():
                value *= factor.get_value([])
        result.add_values([[value]])
    for variable_values in variable_values_lst:
        value = 1
        for factor in Factors:
            if not factor.get_scope():
                value *= factor.get_value([])
            for v in scope:
                if v in factor.get_scope():
                    v.set_assignment(variable_values[scope.index(v)])
            value *= factor.get_value_at_current_assignments()
        variable_values.append(value)
    result.add_values(variable_values_lst)
    return result
...
def restrict_factor(f: Factor, var: Variable, value):
    ...
    scope = f.get_scope()
    scope.remove(var)
    name = "Restrict({}|{}={})".format(scope,var.name, value)
    result = Factor(name, scope)
    # empty scope
    if not scope:
...
def sum_out_variable(f: Factor, var: Variable):
    ...
    scope = f.get_scope()
    scope.remove(var)
    name = "Sum_out({}:{})".format(scope, var.name)
    result = Factor(name, scope)
    # empty scope
    if not scope:
        val = 0
   ...
   Pr(A='c'|B=1, C='c') = 0.26
    ...
    for v in ordering:
        # if v != QueryVar and v not in EvidenceVars:
            ...
            for f in factors:
                if v in f.get_scope():
                    product_lst.append(f)
                    temp.remove(f)
            if product_lst:
                # multiply all relevant factors
                mf = multiply_factors(product_lst)
                # sum out this variable
                sf = sum_out_variable(mf, v)
                temp.append(sf)
            factors = list(temp)
    # Step 3 Normalize
    product = multiply_factors(factors)
    nums = [product.get_value([value]) for value in QueryVar.domain()]
