...
def multiply_factors(Factors):
    ...
    if len(Factors) == 1:
        ...
        return multiply_and_join(Factors[0], Factors[1])
    else:
        newFactors = []
        newFactors.append(multiply_and_join(Factors[0], Factors[1]))
        newFactors += Factors[2:]
        return multiply_factors(newFactors)

def multiply_and_join(f1, f2):
    newName = f1.name + " multiply " + f2.name
    ...
    initial_vals = []  #save and then restore the variable assigned values.
    for v in mergedVars:
        initial_vals.append(v.get_assignment_index())

    multiply_and_join_rec(f1, f2, mergedVars, newValues)

    for v in mergedVars:
        v.set_assignment_index(initial_vals.pop(0))

    newFactor.values = newValues
    return newFactor

def multiply_and_join_rec(f1, f2, mergedVars, newValues):
    if len(mergedVars) == 0:
        ...
        for val in mergedVars[0].domain():
            ...
            multiply_and_join_rec(f1, f2, mergedVars[1:], newValues)
...
def restrict_factor(f, var, value):
    ...
    constant factor'''
    newName = f.name + " restrict " + var.name + " " + value
    newScope = f.get_scope()
    newScope.remove(var)
    newFactor = Factor(newName, newScope)
    newValues = []

    initial_vals = []  #used to restore the variable assigned values.
    for v in f.scope:
        initial_vals.append(v.get_assignment_index())
    ...
    #restoring the value, if not, there will be indexing issues while accessing every row of the distribution
    for v in f.scope:
        v.set_assignment_index(initial_vals.pop(0))
...
def sum_out_variable(f, var):
    ...
    newName = f.name + " sum on " + var.name
    newScope = f.get_scope()
    newScope.remove(var)
    ...
    initial_vals = []  #used to restore the variable assigned values later on
    for v in f.scope:
        initial_vals.append(v.get_assignment_index())

    for val in var.domain():
        var.set_assignment(val)
        sum_recursion(f, newScope, newScopeCopy, newValues)
...
def sum_recursion(oldFactor, newScope, newScopeCopy, newValues):
    if len(newScope) == 0:
        index = 0
        ...
        for val in newScope[0].domain():
            newScope[0].set_assignment(val)
            sum_recursion(oldFactor, newScope[1:], newScopeCopy, newValues)
...
def normalize(nums):
	...
	#if the total sum is 0, we return a list of 0s
	if total_sum == 0:
		new_nums = [0] * len(nums)
		return new_nums
...
def VE(Net, QueryVar, EvidenceVars):
   ...
   Pr(A='a'|B=1, C='c') = 0.26
	...
	#restrict the all the factors in the bays net with the given evidence variables
    for i in range(len(facs)):
        for var in EvidenceVars:
            if var in facs[i].get_scope():
                newFactor = restrict_factor(facs[i], var, var.get_evidence())
                facs[i] = newFactor

    # Decompose the sum and sum out all vars not involved in the query
    mf_ordering = min_fill_ordering(facs, QueryVar)
    for var in mf_ordering:
        ...
        for f in factorsToMultiply:
            facs.remove(f)
        facs.append(summedOutFactor)
        mf_ordering = mf_ordering[1:]

    # Multiply the remaining factors (which only involve the query variable)
    multipliedFactor = multiply_factors(facs)

    # Test final distribution before normalizing. 
    # If it sums to zero, return
