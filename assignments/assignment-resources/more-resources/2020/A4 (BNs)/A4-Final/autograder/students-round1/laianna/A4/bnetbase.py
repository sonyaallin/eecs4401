...
class Factor: 
    ...
    def add_values(self, values):
        ...
        print(self.name, 'new factor')
        for t in values:
            index = 0
            print(t, 'one of the value')
            for v in self.scope:
                index = index * v.domain_size() + v.value_index(t[0])
                print('in factor add now:', t[0],',the index' ,index)
                t = t[1:]
...
def multiply_factors(Factors):
    ...
    result = Factor('first factor', ['a', 'b'])
    return result
...
def restrict_factor(f, var, value):
    ...
    if len(old_scope()) == 1:
        old_scope[0].set_assignment(value)
    ...
    restricted_factor = Factor(f.repr(), new_scope)
...
def normalize(nums):
    ...
    This is used to generate the normalization factor
    '''
    s = sum(nums)
    if s == 0:
        newnums = [0]*len(nums)
