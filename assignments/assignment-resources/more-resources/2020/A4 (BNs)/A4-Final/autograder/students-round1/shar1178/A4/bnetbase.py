...
def multiply_factors(Factors):
    ...
    if len(Factors) == 1:
        ...
        return multiplyhelper(Factors[0], Factors[1])
    else:
        new_factors = []
        new_factors.append(multiplyhelper(Factors[0], Factors[1]))
        new_factors += Factors[2:]
        return multiply_factors(new_factors)

def multiplyhelper(f1, f2):
    newName = f1.name + "_mul_" + f2.name
    ...
    multiplyTwoFacs_rec(f1, f2, mergedVars, newValues)
...
def multiplyTwoFacs_rec(f1, f2, mergedVars, newValues):
    if len(mergedVars) == 0:
        ...
        for val in mergedVars[0].domain():
            ...
            multiplyTwoFacs_rec(f1, f2, mergedVars[1:], newValues)
...
def restrict_factor(f, var, value):
    ...
    saved = []  # save and then restore the variable assigned values.
    for v in f.scope:
        saved.append(v.get_assignment_index())

    var.set_assignment(value)
    restrictRec(f, newScope, newValues)

    for v in f.scope:
        v.set_assignment_index(saved[0])
        saved = saved[1:]
...
def sum_out_variable(f, var):
    ...
    #IMPLEMENT
    newName = f.name + "_sumOut_" + var.name
    newScope = f.get_scope()
    newScope.remove(var)
    newScopeCopy = list(newScope)
    newFac = Factor(newName, newScope)
    ...
    for val in var.domain():
        ...
        sumoutRec(f, newScope, newScopeCopy, newValues)

    for v in f.scope:
        v.set_assignment_index(saved_values[0])
        saved_values = saved_values[1:]

    newFac.values = newValues
    return newFac


def sumoutRec(oldFactor, newScope, newScopeCopy, newValues):
    if len(newScope) == 0:
        index = 0
        ...
        for val in newScope[0].domain():
            newScope[0].set_assignment(val)
            sumoutRec(oldFactor, newScope[1:], newScopeCopy, newValues)
