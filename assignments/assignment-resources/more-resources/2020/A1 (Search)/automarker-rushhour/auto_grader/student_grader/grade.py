...
# CONSTANTS
DEFAULT_ASSIGNMENT_NUM = 1 # Assignment wanting to grade (default if not specified)
...
FILENAME_TEST_CASES = "A1_tests.py"
...
import assignment.A1_tests as test_cases_module
...
def generate_report(assignment_report, directory):
    ...
    for test_case_result in assignment_report.test_case_results:
        if not type(test_case_result.grade) is list: 
            total += test_case_result.grade
    #assignment_report.grade = total

    student_report_body = ['Overall: ' + str(total) + '/' + str(assignment_report.max_grade)]
    for test_case_result in assignment_report.test_case_results:
    ...
    #print("\n====")
    #print("Assignment Grade: {0}".format(assignment_report.grade, assignment_report.max_grade))

    # Print out test case stats
    print('\nTestCase Statistics:')
    for test_case, test_case_result in test_case_reports.items():
        print("\t{0}: ".format(str(test_case)), "{grade}".format(grade=test_case_result.grade))
